...
import { auditoria } from "@/lib/mockData";
...
export function AuditTable() {
  ...
  const filtered = useMemo(() =>
    auditoria.filter(r => r.nome.toLowerCase().includes(q.toLowerCase()) || r.cpf.includes(q)),
  [q]);
    ...
    >
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {pageData.map((r) => (
              <tr key={r.cpf + r.nome} className="border-b border-white/5 text-slate-200 transition-colors hover:bg-amber-500/5">
                ...
                <td className="px-3 py-3">
                  <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] ${statusStyles[r.status]}`}>
                    {r.status}
                  </span>
                </td>
              </tr>
